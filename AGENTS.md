...
- 先批次讀取獨立檔案，再一次性修改；避免同一檔案反覆檢索。已知行號時直接讀取精確區段。
- 每個工作階段的 Git 狀態只在開始、完成與 commit 前檢查；除非有外部變更跡象，不要重複執行 `git status`／`git diff`。
- patch 失敗後先重新讀取精確上下文，再重試一次；不要盲目重放同一補丁。
- 語法、差異與相關測試集中在最後一輪執行；長程序優先使用一次執行搭配合理等待。
- 同一個視覺問題只擷取一次最新畫面：走 8000 統一端點並直接分析回傳檔案；只有使用者表示畫面改變或擷取失敗時才重拍，禁止輪詢舊路徑。
