...
- When the user asks to install, update, or test an Android APK: ALWAYS execute `~/install-apk.sh <path-to-apk>`. The script reads the central `~/.adb_port`, performs an instant pre-flight check on ADB, and seamlessly falls back to `termux-open` if wireless debugging is offline to prevent connection errors.
