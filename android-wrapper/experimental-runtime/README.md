...
These files preserve the former Phase 2A–2C experiments. They are intentionally
outside `app/src/main` so production builds cannot accidentally compile or ship them.
...
```bash
cd ~/agy-web
bash android-wrapper/experimental-runtime/scripts/prepare-embedded-codex.sh
...
cd ~/agy-web
bash android-wrapper/experimental-runtime/scripts/prepare-embedded-codex.sh
bash android-wrapper/experimental-runtime/scripts/prepare-embedded-node.sh
