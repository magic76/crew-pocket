...
function renderStorageReport() {
  ...
  const tabs = `<div class="grid grid-cols-2 gap-2"><button data-storage-tab="conversations" class="storage-tab min-h-11 rounded-xl border text-xs font-bold ${storageTab === 'conversations' ? 'bg-indigo-600 border-indigo-400 text-white' : 'bg-slate-800 border-slate-700 text-slate-300'}">💬 對話 ${conversations.length}</button><button data-storage-tab="media" class="storage-tab min-h-11 rounded-xl border text-xs font-bold ${storageTab === 'media' ? 'bg-indigo-600 border-indigo-400 text-white' : 'bg-slate-800 border-slate-700 text-slate-300'}">🖼️ 圖片／檔案 ${media.length}${storageReport.mediaTruncated ? '+' : ''}</button></div><div class="flex gap-2"><select id="storage-sort" class="flex-1 min-h-11 rounded-xl bg-slate-800 border border-slate-700 px-3 text-xs text-slate-200 outline-none"><option value="updated_desc" ${storageSort === 'updated_desc' ? 'selected' : ''}>最後更新：最新在前</option><option value="updated_asc" ${storageSort === 'updated_asc' ? 'selected' : ''}>最後更新：最舊在前</option><option value="size_desc" ${storageSort === 'size_desc' ? 'selected' : ''}>檔案大小：最大在前</option><option value="size_asc" ${storageSort === 'size_asc' ? 'selected' : ''}>檔案大小：最小在前</option></select><button id="storage-select-all" type="button" class="min-h-11 px-3 rounded-xl bg-slate-800 border border-slate-700 text-[10px] text-slate-300">全選</button></div>`;
  ...
  storageList.querySelector('#storage-select-all')?.addEventListener('click', () => {
    const inputs = [...storageList.querySelectorAll(storageTab === 'conversations' ? '.storage-conversation' : '.storage-media')];
...
async function deleteStorageSelection() {
  ...
  try {
    ...
    storageReport.conversations = (storageReport.conversations || []).filter(item => !conversationInputs.some(input => input.dataset.id === item.id && input.dataset.provider === item.provider));
    storageReport.media = (storageReport.media || []).filter(item => !mediaInputs.some(input => input.dataset.root === item.root && input.dataset.path === item.path));
    renderStorageReport();
