...
function renderAuthRecoveryCard(targetElement, provider, errorMsg, originalMessage = null) {
  ...
  card.innerHTML = `
    ...
    <div class="flex flex-wrap gap-2 pt-1" id="auth-recovery-actions">
      <button type="button" class="px-3 py-1.5 rounded-xl bg-amber-600 hover:bg-amber-500 active:scale-95 text-white font-semibold flex items-center gap-1.5 shadow-lg shadow-amber-900/40 cursor-pointer" onclick="openAuthModal(); if(\'" + provider + "\'===\'codex\') startCodexDeviceFlow();">
        <span>🔑</span>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (authMenuBtn) authMenuBtn.addEventListener("click", openAuthModal);
