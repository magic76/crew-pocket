...
(() => {
  ...
  const en = {
    ...
    '你好！我是你的 Crew Pocket 口袋指揮 🚀': 'Hi! I’m your Crew Pocket command assistant 🚀',
    '我具備': 'I can help with',
    ...
    '保留當前關鍵狀態並繼續工作': 'Keep key current state and continue work',
    '任務接近完成時，只保留重要交接': 'For near-complete work, keep only the essential handoff',
    '一般': 'Standard',
    '保留當前關鍵狀態以繼續工作；任務接近完成時可用': 'keeps the key current state so work can continue. When a task is nearly complete, use',
    '，只留重要交接並降低 Context。': 'to keep only the essential handoff and reduce context.',
    '已最佳化壓縮': 'Optimized',
    '移除圖片': 'Remove image',
    ...
    '口袋指揮超能力功能錦囊': 'Pocket command capabilities',
    'Termux 行動端專屬強大能力全覽': 'A guide to Crew Pocket’s mobile capabilities',
    '1. Gemini Live 語音助理': '1. Gemini Live voice assistant',
    '即時雙向通話，支援音色、個人 Prompt、聲紋門檻、插話策略與通話保護；可視情況帶入最近主對話脈絡。': 'Real-time two-way calls with voice, personal prompt, voiceprint threshold, interruption strategy, and call protection. Recent main-chat context can be included when needed.',
    '2. Live 委派主對話': '2. Delegate from Live to main chat',
    '在通話中說明要做的事，確認後由主對話處理；任務中心會保留進度、結果、失敗原因，並可重試。': 'Describe work during a call, then confirm for the main chat to handle it. Task Center keeps progress, results, errors, and retries.',
    '3. 相機與手機感知': '3. Camera and phone awareness',
    '明確要求時才讀取目前螢幕或拍攝實體環境；Live 可擷取當下相機畫面交給主對話，避免誤用舊照片。': 'The app reads your screen or captures the physical scene only when explicitly requested. Live can hand the current camera frame to main chat, avoiding stale photos.',
    '4. 對話保存與記憶整理': '4. Conversation archive and memory',
    '語音通話會保存為備忘錄；使用': 'Voice calls are saved as memos. Use',
    '精簡上下文時，完整歷史、圖片與卡片仍可從對話紀錄還原。': 'to compact context while full history, images, and cards remain recoverable from conversation history.',
    '5. 可安裝 PWA 與更新保護': '5. Installable PWA and update protection',
    '可安裝為獨立 App；核心介面會在首次成功開啟後快取，版本更新會自動接管並重新載入，避免讀到舊程式。': 'Install it as a standalone app. The core UI is cached after the first successful load; updates take over and reload automatically so old code is not used.',
    '6. 行動工作台': '6. Mobile workspace',
    '瀏覽 Termux 檔案、管理對話與媒體空間、執行程式碼，並可在對話中直接預覽 HTML、圖表與 Google 地圖導航。': 'Browse Termux files, manage conversation and media storage, run code, and preview HTML, charts, and Google Maps navigation directly in chat.',
    ...
    '可提煉核心脈絡，所有歷史對話仍完好保留在畫面上！': 'to retain the core context; all history remains visible.',
    ...
    '極致壓縮 /compact-max': 'Max compact /compact-max',
    '任務接近完成時使用：只保留最終成果、待驗證事項與重要交接。': 'Use when a task is nearly complete: keep only final outcomes, remaining verification, and the essential handoff.',
