...
// Keep directory listings responsive on a phone. Large trees report a lower
// bound instead of walking every descendant before the list can render.
async function getDirectorySize(directoryPath, maxEntries = 750) {
  let total = 0;
  let visited = 0;
  ...
  while (pending.length > 0 && visited < maxEntries) {
    ...
    for (const entry of entries) {
      ...
      try { total += (await fsPromises.stat(entryPath)).size; }
      catch (_) {}
    }
  }
  return { bytes: total, partial: pending.length > 0 || visited >= maxEntries };
}
...
async function handleListFiles(parsedUrl, res) {
  try {
    ...
    for (const ent of dirEntries) {
      ...
      if (isDir) {
        const directorySize = await getDirectorySize(fullPath);
        size = directorySize.bytes;
        sizePartial = directorySize.partial;
      }
      ...
      entries.push({
        ...
        isDirectory: isDir,
        size,
        sizeFormatted: `${sizePartial ? '≥ ' : ''}${formatBytes(size)}`,
        sizePartial,
        mtime,
        icon: meta.icon,
        type: meta.type,
...
// 📋 Copy or move an explorer item into an existing folder under Termux Home.
async function handleTransferFile(req, res) {
  try {
    const { parseJsonBody } = require('./config');
    const body = await parseJsonBody(req);
    const action = body.action === 'move' ? 'move' : body.action === 'copy' ? 'copy' : '';
    ...
    if (!action || !sourcePath || destinationPath === undefined) throw new Error('缺少檔案操作資料');

    const source = path.resolve(HOME_DIR, sourcePath.replace(/^[\\/]+/, ''));
    const destinationDir = path.resolve(HOME_DIR, destinationPath.replace(/^[\\/]+/, ''));
    const sourceRel = path.relative(HOME_DIR, source);
    const destinationRel = path.relative(HOME_DIR, destinationDir);
    if (!sourceRel || sourceRel.startsWith('..') || path.isAbsolute(sourceRel)
      || destinationRel.startsWith('..') || path.isAbsolute(destinationRel)) {
      const error = new Error('安全限制：只能操作 Home 目錄及其 Download 連結內的項目');
      error.statusCode = 403;
      throw error;
    }

    const [sourceStat, destinationStat] = await Promise.all([fsPromises.lstat(source), fsPromises.stat(destinationDir)]);
    if (!destinationStat.isDirectory()) throw new Error('目標必須是資料夾');
    const target = path.join(destinationDir, path.basename(source));
    if (target === source || (sourceStat.isDirectory() && target.startsWith(`${source}${path.sep}`))) {
      throw new Error('不能將資料夾操作到自己或其子資料夾');
    }
    try {
      await fsPromises.lstat(target);
      throw new Error(`目標已存在同名項目：${path.basename(target)}`);
    } catch (error) {
      if (error.code !== 'ENOENT') throw error;
    }

    if (action === 'copy') {
      await fsPromises.cp(source, target, { recursive: sourceStat.isDirectory(), errorOnExist: true, force: false });
    } else {
      try {
        ...
        await fsPromises.cp(source, target, { recursive: sourceStat.isDirectory(), errorOnExist: true, force: false });
        await fsPromises.rm(source, { recursive: sourceStat.isDirectory(), force: false });
      }
    }

    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ success: true, action, targetPath: path.relative(HOME_DIR, target) }));
  } catch (err) {
    res.writeHead(err.statusCode || (err.code === 'ENOENT' ? 404 : 400), { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ success: false, error: err.message }));
  }
