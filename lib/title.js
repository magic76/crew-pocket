...
function generateTitleWithAI(userMessage, assistantResponse, locale = 'zh-TW') {
  return new Promise((resolve, reject) => {
    const context = `User: ${userMessage.slice(0, 200)}\nAssistant: ${(assistantResponse || '').slice(0, 300)}`;
    const isEnglish = locale === 'en';
    const systemPrompt = isEnglish
      ? 'You generate conversation titles. Based on the conversation below, produce a concise English title of 3–8 words. Do not add quotes, punctuation, or any other text; output only the title.'
      : '你是一個對話標題生成器。根據以下對話內容，用繁體中文生成一個精簡的對話標題（5-10個字，不要加引號或符號）。只輸出標題文字，不要輸出任何其他內容。';
...
async function handleGenerateTitle(req, res) {
  try {
    ...
    // Generate with AI
    const title = await generateTitleWithAI(user_message, assistant_response || '', body.locale === 'en' ? 'en' : 'zh-TW');
