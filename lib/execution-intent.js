...
function isHighRiskExecutionPrompt(prompt) {
  ...
  return /(?:\b(?:build|release|deploy|publish|migration|migrate|dependency|dependencies|refactor|rewrite|restructure|upgrade|downgrade|multi[- ]?(?:file|module|repo)|whole project|entire project)\b|建置|打包|發布|發佈|上架|部署|遷移|移轉|依賴|相依|重構|重寫|全面|整個專案|多檔|多模組)/i.test(text);
}
