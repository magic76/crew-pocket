const { spawnAgy } = require('./runtime/agy-transport');
...
class ActiveSessionManager {
  ...
  async getOrCreateSession(targetConversationId, targetModel, targetEffort = 'low', targetWorkspace = process.env.HOME || '/data/data/com.termux/files/home') {
    ...
    if (targetConversationId && this.sessions.has(targetConversationId)) {
      ...
      if (existing.process && !existing.process.killed) {
        ...
        const workspaceMatches = existing.workspace === targetWorkspace;
        if (modelMatches && effortMatches && workspaceMatches) {
          ...
          // Model, effort, or workspace changed for this conversation — restart
          // so the process cannot retain the previous project's CWD.
    ...
    if (!targetConversationId && this.sessions.has('__standby__')) {
      ...
      if (standby.process && !standby.process.killed && (!targetModel || standby.model === targetModel) && (!targetEffort || standby.effort === targetEffort) && standby.workspace === targetWorkspace && !standby.isBusy) {
    ...
    console.log(`[SessionManager] Spawning resident agy process for session: ${targetConversationId || 'NEW'} (Model: ${targetModel || 'default'}, Workspace: ${targetWorkspace}) (Pool: ${this.sessions.size}/${MAX_SESSIONS})`);
    ...
    // Only pass --effort for models that support it (e.g. gemini models)
    // Claude (claude-opus-4-6-thinking, claude-sonnet-4-6) and GPT-OSS do NOT support --effort in agy CLI
    const supportsEffort = !targetModel || targetModel.startsWith('gemini');
    if (targetEffort && supportsEffort) {
  ...
  prewarm(targetModel = 'gemini-3.7-flash', targetEffort = 'low', targetWorkspace = process.env.HOME || '/data/data/com.termux/files/home') {
    targetModel = targetModel || 'gemini-3.7-flash';
    targetEffort = targetEffort || 'low';
    const key = String(targetModel) + ':' + String(targetEffort) + ':' + String(targetWorkspace);
    const now = Date.now();
    ...
    const standby = this.sessions.get('__standby__');
    if (standby) {
      const matches = standby.model === targetModel && standby.effort === targetEffort && standby.workspace === targetWorkspace;
    ...
    console.log('[SessionManager] 🔥 Pre-warming background standby session (Model: ' + targetModel + ', Workspace: ' + targetWorkspace + ')...');
    this.prewarmInFlight = this.getOrCreateSession(null, targetModel, targetEffort, targetWorkspace)
      .then(() => ({ started: true, model: targetModel, effort: targetEffort, workspace: targetWorkspace }))
      .catch((err) => {
  ...
  resetIdleTimer(session) {
    ...
    session.idleTimer = setTimeout(() => {
      console.log(`[SessionManager] Session ${session.conversationId} idle for ${IDLE_TIMEOUT_MS / 60000}m, sleeping to save battery/RAM. (Pool: ${this.sessions.size - 1}/${MAX_SESSIONS})`);
