...
class PhoneAgentController {
  ...
  async connectWireless(port, host = '127.0.0.1') {
    return new Promise((resolve) => {
      const target = `${host}:${port}`;
      exec(`adb connect ${target}`, (err, stdout, stderr) => {
        const out = (stdout || stderr || '').trim();
        const success = out.includes('connected to') && !out.includes('unable');
        resolve({ success, output: out, target });
      });
    });
  }

  async pairWireless(port, pairingCode, host = '127.0.0.1') {
    return new Promise((resolve) => {
      const target = `${host}:${port}`;
      exec(`adb pair ${target} ${pairingCode}`, (err, stdout, stderr) => {
        const out = (stdout || stderr || '').trim();
