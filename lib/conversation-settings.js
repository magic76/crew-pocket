...
async function saveConversationSettings(provider, conversationId, settings) {
  ...
  const data = await readAll();
  const value = { ...(data[key(provider, conversationId)] || {}), provider, model: settings.model, effort: settings.effort, updatedAt: Date.now() };
...
module.exports = { getConversationSettings, saveConversationSettings, saveConversationTitle, getConversationTitles, deleteConversationSettings };
