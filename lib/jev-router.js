...
const { isHighRiskExecutionPrompt } = require('./execution-intent');
...
function shouldRouteWithJev({ provider, model, explicitExecutionMode, prompt }) {
  ...
  if (!isLunaModel(model)) return false;
  if (explicitExecutionMode) return false;
  if (!String(prompt || '').trim()) return false;
  return !looksLikeContinuation(prompt) && isHighRiskExecutionPrompt(prompt);
}
