...
async function reviewExecutionIntentWithJev({
  ...
  const questions = JSON.stringify({
    intent_review: {
      type: 'choice',
      instructions: [
        'Review whether this execution intent is justified by the ORIGINAL task.',
        'Do not legitimize unrelated work just because the agent proposed it.',
        'KEEP_CURRENT means stay inside the current policy and drop any unnecessary over-scope.',
        ...
        'REJECT_SCOPE when the intent adds unnecessary work outside the user request.'
      ].join(' '),
