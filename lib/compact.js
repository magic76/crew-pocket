...
async function handleCompact(req, res) {
  try {
    ...
    if (historySegments.length < 2) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: isEnglish ? 'Conversation is too short to compact (at least one complete turn is required).' : '對話記錄過短（需至少 1 輪完整問答），無需壓縮' }));
    }

    // AGY appends new turns to transcript.jsonl after every checkpoint.  Fold
    // those turns into the immutable UI archive before this compact replaces
    // the active transcript with a lightweight snapshot.
    ...
    const fullHistoryText = buildCompactionSource(historySegments);
    console.log(`[Compact] Generating ${mode} compaction for conversation ${conversation_id} (${historySegments.length} segments, ${fullHistoryText.length} chars)...`);

    const summary = await generateCompactedSummary(fullHistoryText, focus, isEnglish ? 'en' : 'zh-TW', mode);
    ...
    // Append checkpoint to transcript so past messages are 100% preserved visually,
    // while agy CLI starts its context window from the CHECKPOINT record!
    ...
    const modelCompactStep = JSON.stringify({
      ...
      created_at: nowIso,
      content: isEnglish
        ? `📦 **Conversation memory ${mode === 'max' ? 'max-compacted' : 'compacted'} successfully!**\n\n${summary}`
        : `📦 **對話記憶已${mode === 'max' ? '極致' : '成功'}精簡壓縮！**\n\n${summary}`
    });

    // ⚡ Scheme A: Archival & Lightweight Snapshot Truncation
    // transcript_full.jsonl has already been incrementally synchronized above.
    // Append only this newly-created compact checkpoint to its immutable audit trail.
    const appendChunk = `\n${checkpointStep}\n${userCompactStep}\n${modelCompactStep}\n`;
    await fsPromises.appendFile(logFullPath, appendChunk);

    // 2. Truncate transcript.jsonl to ONLY the lightweight Checkpoint snapshot!
    // This allows agy CLI cold starts to deserialize in < 50ms instead of 80s!
    const cleanSnapshot = `${checkpointStep}\n${userCompactStep}\n${modelCompactStep}\n`;
    await fsPromises.writeFile(logPath, cleanSnapshot, 'utf-8');

    // Close resident session so it reloads instantly from the new clean snapshot
    ...
    res.end(JSON.stringify({
      ...
      summary,
      message: isEnglish
        ? (mode === 'max' ? 'Conversation memory max-compacted successfully!' : 'Conversation memory compacted successfully!')
        : (mode === 'max' ? '對話記憶已極致精簡壓縮！' : '對話記憶已成功精簡壓縮！')
    }));

  } catch (err) {
    console.error('[Compact Error]', err);
    res.writeHead(500, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: err.message || 'Failed to compact conversation memory' }));
  }
}
