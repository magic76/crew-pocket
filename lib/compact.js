...
function generateCompactedSummary(conversationText, userFocus, locale = 'zh-TW') {
...
async function handleCompact(req, res) {
  try {
    ...
    const summary = await generateCompactedSummary(fullHistoryText, focus, isEnglish ? 'en' : 'zh-TW');
    ...
    const modelCompactStep = JSON.stringify({
      ...
      status: "DONE",
      created_at: nowIso,
      content: isEnglish ? `📦 **Conversation memory compacted successfully!**\n\n${summary}` : `📦 **對話記憶已成功精簡壓縮！**\n\n${summary}`
    });
    ...
    res.end(JSON.stringify({
      ...
      conversation_id,
      summary,
      message: isEnglish ? 'Conversation memory compacted successfully!' : '對話記憶已成功精簡壓縮！'
    }));
