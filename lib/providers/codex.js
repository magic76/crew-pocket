...
class CodexProvider {
  ...
  async listConversations() {
    ...
    // Sidebar rows only need lightweight thread metadata. Reading every session JSONL
    // for turn and token totals makes opening the drawer slow on large projects.
  ...
  async findSessionPath(threadId) {
    const indexed = this.threadIndex.get(threadId);
    if (indexed?.path) {
      const indexedPath = path.resolve(indexed.path);
      if (indexedPath.startsWith(`${CODEX_SESSIONS_DIR}${path.sep}`)) return indexedPath;
    }
    ...
    while (pending.length > 0) {
      ...
      for (const entry of entries) {
        ...
        else if (entry.isFile() && entry.name.endsWith('.jsonl') && entry.name.includes(threadId)) return entryPath;
      }
    }
    return null;
  }
  ...
  async getLocalHistory(threadId) {
    ...
    if (!sessionPath || !sessionPath.startsWith(`${CODEX_SESSIONS_DIR}${path.sep}`)) return null;
