...
function guardProviderTurns(provider) {
  ...
  provider.startTurn = async function startGuardedTurn(args = {}) {
    ...
    try {
      ...
      // Codex resolves startTurn only after turn_completed, at which point this
      // guard has already released the lock. Do not re-acquire it on return.
      if (!released && result?.conversationId && !claim(result.conversationId)) {
  ...
  provider.stop = async function stopGuardedTurn(conversationId = null) {
    ...
    if (!id) {
      const activeIds = [...locks.keys()];
      if (activeIds.length === 1) {
        id = activeIds[0];
      } else if (activeIds.length > 1) {
        const error = new Error('Multiple conversations are active; conversation_id is required');
        error.statusCode = 409;
        error.code = 'AMBIGUOUS_STOP_TARGET';
        throw error;
      } else {
        return { stopped: false, conversationId: null };
      }
    }
