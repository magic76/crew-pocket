...
async function createTask(input = {}) {
  return mutateTasks(tasks => {
    ...
    const task = {
      ...
      conversationTitle: cleanText(input.conversationTitle, 160),
...
async function updateTask(id, patch = {}, event = null) {
  return mutateTasks(tasks => {
    ...
    if (!task) return null;
    const allowed = ['status', 'provider', 'conversationId', 'conversationTitle', 'model', 'effort', 'title', 'task', 'result', 'error'];
