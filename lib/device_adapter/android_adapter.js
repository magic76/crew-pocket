...
class AndroidDeviceAdapter extends DeviceAdapter {
  ...
  // Tier 1: App Launch with AppRegistry resolution & Verification
  async launchApp(target) {
    return this.executeWithVerification('launchApp', async () => {
      let appName = '';
      let pkg = '';
      let url = '';
      ...
      if (resp && resp.success) {
        return createActionResult('launchApp', 'os', true, '已成功啟動 ' + targetLabel, {
  ...
  async pressKey(keyName) {
    ...
    return this.executeWithVerification('pressKey', async () => {
  ...
  async openUrl(url) {
    return this.executeWithVerification('openUrl', async () => {
      ...
      if (!before || !after) return true;
      return before.packageName !== after.packageName || JSON.stringify(before.elements) !== JSON.stringify(after.elements);
    }, 0);
  }

  // Tier 2: Semantic Tap with 3-Level Fallback & Verification
  async tap(target) {
    return this.executeWithVerification('tap', async () => {
  ...
  // Tier 2: Semantic Scroll & Verification
  async scroll(direction = 'up', distance = 'normal', containerId = null) {
    return this.executeWithVerification('scroll', async () => {
      const dir = String(direction || 'up').toLowerCase();
      const resp = await this.sendRequest('/scroll', { direction: dir, distance, id: containerId });
  ...
  // Tier 2: Proportional Gesture Swipe & Verification
  async swipe(direction = 'up', distance = 'normal') {
    return this.executeWithVerification('swipe', async () => {
