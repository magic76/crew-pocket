        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (item.type === 'USER_INPUT' && item.content) {
              ...
              if (cleaned) {
                // Ignore internal one-shot system tasks
            ...
            } else if (item.type === 'CHECKPOINT' && item.content) {
              // 🌟 Extract title from compacted conversation checkpoint
              const mObj = item.content.match(/# USER Objective:\s*([^\n]+)/);
              if (mObj && mObj[1].trim()) {
                hasValidUserMessage = true;
                if (!title) title = mObj[1].trim();
                break;
              }
              const mReq = item.content.match(/1\.\s*([^\n]+)/);
              if (mReq && !mReq[1].startsWith('你是一個專業')) {
                const cleanedReq = mReq[1].replace(/\[Context:[^\]]+\]/g, '').replace(/\[Uploaded Image:[^\]]+\]/g, '').trim();
                if (cleanedReq) {
                  hasValidUserMessage = true;
                  if (!title) title = cleanedReq.slice(0, 35) + (cleanedReq.length > 35 ? '...' : '');
                  break;
                }
              }
              const mGoal = item.content.match(/-\s*\*\*([^\*]+)\*\*：([^\n]+)/);
              if (mGoal) {
                hasValidUserMessage = true;
                if (!title) title = (mGoal[1] + ': ' + mGoal[2]).slice(0, 35);
                break;
              }
            }
          } catch (e) {}
