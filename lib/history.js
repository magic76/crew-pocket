  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      try {
        ...
        // Scan transcript for real user messages and calculate context stats
        const content = await fsPromises.readFile(logPath, 'utf-8');
        ...
        if (hasValidUserMessage || cachedTitle) {
          const stats = calculateContextStats(lines);
          convList.push({
            id: convId,
            ...
            status_level: stats.status_level,
            is_compacted: stats.is_compacted
          });
        }
      } catch (err) {}
    }
