...
async function handleListConversations(res) {
  try {
    ...
    convList.sort((a, b) => (a.title || '').localeCompare(b.title || '', 'zh-TW') || a.id.localeCompare(b.id));
    res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (item.type === 'CHECKPOINT') {
          ...
          const match = checkpointSummary.match(/# Compacted Conversation Context[^\n]*\s*\n+([\s\S]*)/i);
          if (match) checkpointSummary = match[1];
          messages.push({
            role: 'checkpoint',
            content: checkpointSummary.trim() || item.content || '',
          ...
          if (item.content && /^📦 \*\*對話記憶已(?:成功|極致)精簡壓縮！/.test(item.content) && messages.length > 0 && messages[messages.length - 1].role === 'checkpoint') {
            continue;
          }
